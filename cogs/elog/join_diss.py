...
class Join_Diss(commands.Cog):
    ...
    async def on_member_join(self, member):
        ...
        result = int(db.get_channel_id())
        if result:
            db.update_members_join_at_user_id(joined_at, member.id)
        else:
            db.insert_members_join(member.id, joined_at, '')
    ...
    async def on_member_remove(self, member):
        ...
        if joined_at:
            ...
            db.update_members_left_at(left_at, member.id)
